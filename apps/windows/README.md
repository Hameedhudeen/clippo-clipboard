...
The shell has a WinForms app target, system tray icon, global hotkey registration, Windows clipboard listener, searchable history popup, keyboard and pointer selection, numbered selection shortcuts, pinned-item shortcut labels, wrapped footer buttons and tray menu items for the common core commands, local AppData history persistence, copy/paste actions through Windows APIs, Windows Clipboard History coexistence safeguards, UAC-aware fallback when the foreground target is elevated, pause/ignore actions, `Ctrl+Comma` preferences access, one-time first-run Launch at login prompt, launch-at-login registration, popup/preference windows that hide instead of stopping the tray process, DPI-aware forms, and tray notifications.
