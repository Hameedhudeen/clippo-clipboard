...
- Maccy-like compact history popup adapted to Windows conventions.
- Windows clipboard APIs and input simulation.
- Rust shared core through a narrow FFI boundary.

## Current Status

The shell has a WinForms app target, system tray icon, global hotkey registration, Windows clipboard listener, searchable history popup, keyboard and pointer selection, numbered selection shortcuts, pinned-item shortcut labels, wrapped footer buttons and tray menu items for the common Maccy-like commands, copy/paste actions through Windows APIs, Windows Clipboard History coexistence safeguards, UAC-aware fallback when the foreground target is elevated, pause/ignore actions, `Ctrl+Comma` preferences access, launch-at-login registration, DPI-aware forms, and tray notifications.
